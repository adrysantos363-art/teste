...
$("loginBtn").addEventListener("click",async()=>{
  ...
  try{
    if(!connectFirebase()){
      $("loginError").textContent="Configure o Firebase na aba Configurações primeiro.";
      return;
    }
    await firebaseAuth.signInWithEmailAndPassword(email,password);
    if(firebaseAuth.currentUser.uid!==ADMIN_UID){
      await firebaseAuth.signOut();
      $("loginError").textContent="Esta conta não está autorizada para administrar a loja.";
      return;
    }
    $("loginError").textContent="";
    closeModal("loginModal"); openModal("adminModal"); renderAdmin(); loadSettings();
    setFirebaseStatus("Administrador conectado. Alterações serão salvas no Firebase.");
  }catch(error){
    console.error(error);
    $("loginError").textContent="Não foi possível entrar: "+(error.message||"verifique as credenciais.");
  }
});
$("adminPassword").addEventListener("keydown",e=>{if(e.key==="Enter")$("loginBtn").click()});
$("logoutBtn").addEventListener("click",async()=>{if(firebaseAuth) await firebaseAuth.signOut(); closeModal("adminModal");});
...
window.deleteProduct=async id=>{
  ...
  try{
    if(!firestoreDb || !firebaseAuth || !firebaseAuth.currentUser) throw new Error("Faça login no Firebase.");
    if(firebaseAuth.currentUser.uid!==ADMIN_UID) throw new Error("Usuário sem permissão.");
    await firestoreDb.collection("products").doc(String(id)).delete();
    products=products.filter(p=>p.id!==id); saveProducts(); renderProducts(); renderAdmin();
    setFirebaseStatus("Produto excluído e removido do Firebase.");
...
$("productForm").addEventListener("submit",async e=>{
  ...
  if(!data.name || !Number.isFinite(data.price)){alert("Preencha nome e preço corretamente.");return;}
  try{
    if(!firestoreDb || !firebaseAuth || !firebaseAuth.currentUser){
      if(!connectFirebase()) throw new Error("Firebase não configurado.");
    }
    if(firebaseAuth.currentUser.uid!==ADMIN_UID) throw new Error("Usuário sem permissão de administrador.");
    await firestoreDb.collection("products").doc(String(data.id)).set(data);
    products=editingId?products.map(p=>p.id===editingId?data:p):[...products,data];
    saveProducts(); renderProducts(); renderAdmin(); resetForm();
    setFirebaseStatus("Produto salvo no Firebase e sincronizado com os outros aparelhos.");
  }catch(error){
    console.error(error);
    alert("Não foi possível salvar no Firebase: "+error.message);
    setFirebaseStatus("Falha ao salvar: "+error.message,true);
  }
});
...
$("saveSettings").addEventListener("click",()=>{
  settings={
    ...
    firebase:{
      ...
      projectId:$("fbProjectId").value.trim() || FIREBASE_PROJECT_ID,
...
const FIREBASE_PROJECT_ID = "thimports-1ce81";
const ADMIN_UID = "DqPq8jyUcNOfWrHYFWgssKHgCh93";
...
function connectFirebase(){
  try{
    if(typeof firebase==="undefined"){
      setFirebaseStatus("SDK do Firebase não carregado. Abra pelo GitHub Pages/Live Server.",true);
      return false;
    }
    const config=getFirebaseConfig();
    if(!validFirebaseConfig(config)){
      setFirebaseStatus("Preencha apiKey, authDomain, projectId e appId em Configurações.",true);
      return false;
    }
...
$("uploadFirebase").addEventListener("click",async()=>{
  ...
  if(!firebaseAuth?.currentUser || firebaseAuth.currentUser.uid!==ADMIN_UID){setFirebaseStatus("Faça login com o administrador do Firebase.",true);return;}
  try{
...
/* =========================================================
   V3 — SINCRONIZAÇÃO MULTI-APARELHO
   O catálogo público usa somente o projectId e a API REST do
   Firestore. Assim, celular e computador não dependem do
   localStorage nem de uma configuração salva em outro aparelho.
   ========================================================= */
function fsValue(v){
  if(!v) return null;
  ...
  if("mapValue" in v){const o={};Object.entries(v.mapValue.fields||{}).forEach(([k,x])=>o[k]=fsValue(x));return o;}
  return null;
}
function fsDoc(doc){
  const out={};
  Object.entries(doc.fields||{}).forEach(([k,v])=>out[k]=fsValue(v));
  if(out.id==null) out.id=doc.name.split("/").pop();
  return out;
}
async function publicFirestoreGet(collection){
  const url=`https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/(default)/documents/${collection}?pageSize=1000`;
...
async function syncPublicCatalog(){
  try{
    const data=await publicFirestoreGet("products");
    const remote=(data.documents||[]).map(fsDoc).filter(p=>p.name);
    if(remote.length){
      products=remote.map(p=>({...p,id:Number(p.id)||p.id}));
      saveProducts(); renderProducts(); renderAdmin();
      const status=$("firebaseStatus");
      if(status && document.getElementById("adminModal") && !document.getElementById("adminModal").classList.contains("hidden")){
        status.textContent="Catálogo sincronizado com o Firebase.";
      }
    }
    try{
      const settingsUrl=`https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/(default)/documents/store/settings`;
      const sr=await fetch(settingsUrl,{cache:"no-store"});
      if(sr.ok){
        const sd=await sr.json();
        if(sd.fields){
          const remoteSettings=fsDoc(sd);
          settings={...settings,...remoteSettings};
          saveSettings(); renderCart();
        }
      }
    }catch(e){/* configurações públicas são opcionais */}
  }catch(error){
    console.warn("Sincronização pública indisponível:",error);
  }
}
function startPublicSync(){
  syncPublicCatalog();
  if(publicSyncTimer) clearInterval(publicSyncTimer);
  publicSyncTimer=setInterval(syncPublicCatalog,5000);
}
startPublicSync();
